...
typedef struct { // WS2355 FIFO Of Payload
  Payload    payload;        // the payload Data
  unsigned long updated;   // Last Updated Time of this Payloaf Entry
  bool sent;  // If False then not sent correctly
} WS2355_AOP;

#define MAX_FIFO_WS2355_AOP 12
int FIFO_WS2355_AOP_PUSH = 0;
int FIFO_WS2355_AOP_POP = 0;

WS2355_AOP FIFO_WS2355_AOP[MAX_FIFO_WS2355_AOP];
...
void setup(void)
{
  ...
  payload.data.SENSOR_STATION.Station_id = 199;
  ...
  for (i = 0; i < MAX_FIFO_WS2355_AOP; i++) {
    ...
    FIFO_WS2355_AOP[i].payload.type =  SENSOR_STATION;
    FIFO_WS2355_AOP[i].payload.Version = PAYLOAD_VERSION; // initialize data for all payload -- PAYLOAD_VERSION; from sensor_payload.h
    FIFO_WS2355_AOP[i].payload.Station_id = STATION_NODE; // initialize data for all payload
    FIFO_WS2355_AOP[i].payload.Node = 0; // Master primary
    //printf("Fifo initialized %u %u",i,FIFO_WS2355_AOP[i].sent);
  }
  printf("//RF Module Sensor Enabled.\n");
  Init_Ports();
...
void loop(void)
{
  ...
  for (int ti = FIFO_WS2355_AOP_PUSH; ti < (FIFO_WS2355_AOP_PUSH + MAX_FIFO_WS2355_AOP); ti++ ) {
    i = ti % MAX_FIFO_WS2355_AOP;
    if ((FIFO_WS2355_AOP[i].sent == false) && (millis() - FIFO_WS2355_AOP[i].updated > 10000))  {
      //printf("DEBUG FIFO_WS2355_AOP_PUSH=%u / MAX_FIFO_WS2355_AOP=%u\n",FIFO_WS2355_AOP_PUSH,MAX_FIFO_WS2355_AOP);
      //printf("DEBUG i loop i=%u\n",i);
      // We relay information 10 second after having decoded the packet
      // We check if another similar packet exist in the stack and remove it if redundant
      //#ifdef DEBUG
      //Serial.print("Found candidate at index "); Serial.println(i);
      //#endif
      int j;
      for (int tj = FIFO_WS2355_AOP_PUSH; tj < (FIFO_WS2355_AOP_PUSH + MAX_FIFO_WS2355_AOP); tj++ ) {
        j = tj % MAX_FIFO_WS2355_AOP;
        //printf("DEBUG j loop j=%u\n",j);
        if ((FIFO_WS2355_AOP[j].sent == false) && (i != j) && (FIFO_WS2355_AOP[j].payload.type == FIFO_WS2355_AOP[i].payload.type)
            && (FIFO_WS2355_AOP[j].payload.Station_id == FIFO_WS2355_AOP[i].payload.Station_id)) {
          //printf("Skip %u due to %u\n",j,i);
          FIFO_WS2355_AOP[j].sent = true; // Force sent = true;
        }
      }
...
--------------------------------------------------------------------------------------*/
bool Packet_Converter_WS2355(void)
{
  byte b;
  byte c;
  sint si;
        ...
        case 0:
          {
            ...
            FIFO_WS2355_AOP[FIFO_WS2355_AOP_PUSH].payload.type = WS2355_TEMP;
            FIFO_WS2355_AOP[FIFO_WS2355_AOP_PUSH].payload.Node = 10; // Master primary
            FIFO_WS2355_AOP[FIFO_WS2355_AOP_PUSH].payload.data.WS2355_TEMP.temperature = (float) siWSR_CurrentTemperature / 10.0 ;
            FIFO_WS2355_AOP[FIFO_WS2355_AOP_PUSH].payload.data.WS2355_TEMP.status = 1;
            FIFO_WS2355_AOP[FIFO_WS2355_AOP_PUSH].sent = false;
            FIFO_WS2355_AOP[FIFO_WS2355_AOP_PUSH].updated = millis();
            Serial.println("WS2355_TEMP SENSOR");
            Serial.print("> Temp  ="); Serial.println(FIFO_WS2355_AOP[FIFO_WS2355_AOP_PUSH].payload.data.WS2355_TEMP.temperature, DEC);
            FIFO_WS2355_AOP_PUSH++;
            break;
          }
        case 1:
          {
            ...
            FIFO_WS2355_AOP[FIFO_WS2355_AOP_PUSH].payload.type = WS2355_HUM;
            FIFO_WS2355_AOP[FIFO_WS2355_AOP_PUSH].payload.Node = 10; // Master primary
            FIFO_WS2355_AOP[FIFO_WS2355_AOP_PUSH].payload.data.WS2355_HUM.humidity = bWSR_CurrentHumidity; ;
            FIFO_WS2355_AOP[FIFO_WS2355_AOP_PUSH].payload.data.WS2355_HUM.status = 1;
            FIFO_WS2355_AOP[FIFO_WS2355_AOP_PUSH].sent = false;
            FIFO_WS2355_AOP[FIFO_WS2355_AOP_PUSH].updated = millis();
            Serial.println("WS2355_HUM SENSOR");
            Serial.print("> Humidity  ="); Serial.println(FIFO_WS2355_AOP[FIFO_WS2355_AOP_PUSH].payload.data.WS2355_HUM.humidity, DEC);
            //doSendMsg();
            FIFO_WS2355_AOP_PUSH++;
            break;
          }
        case 2:
          {
            ...
            //Serial.print("uiWSR_RainfallCount=");Serial.println(uiWSR_RainfallCount,DEC);
            //Serial.print("uiWSR_Rainfallmm=");Serial.println((float) (uiWSR_RainfallCount * 0.518),DEC);
            FIFO_WS2355_AOP[FIFO_WS2355_AOP_PUSH].payload.type = WS2355_RAIN;
            FIFO_WS2355_AOP[FIFO_WS2355_AOP_PUSH].payload.Node = 10; // Master primary
            FIFO_WS2355_AOP[FIFO_WS2355_AOP_PUSH].payload.data.WS2355_RAIN.totalrain = (float) uiWSR_RainfallCount * 0.518;
            FIFO_WS2355_AOP[FIFO_WS2355_AOP_PUSH].payload.data.WS2355_RAIN.status = 1;
            FIFO_WS2355_AOP[FIFO_WS2355_AOP_PUSH].sent = false;
            FIFO_WS2355_AOP[FIFO_WS2355_AOP_PUSH].updated = millis();
            Serial.println("WS2355_RAIN SENSOR");
            Serial.print("> Total rain  ="); Serial.println(FIFO_WS2355_AOP[FIFO_WS2355_AOP_PUSH].payload.data.WS2355_RAIN.totalrain, DEC);
            FIFO_WS2355_AOP_PUSH++;
            break;
          }
        case 3:
...
#endif
            FIFO_WS2355_AOP[FIFO_WS2355_AOP_PUSH].payload.type = WS2355_WIND;
            FIFO_WS2355_AOP[FIFO_WS2355_AOP_PUSH].payload.Node = 10; // Master primary
            FIFO_WS2355_AOP[FIFO_WS2355_AOP_PUSH].payload.data.WS2355_WIND.speed = (float) uiWSR_CurrentWindSpeed_m_per_sec / 10.0 ;
            FIFO_WS2355_AOP[FIFO_WS2355_AOP_PUSH].payload.data.WS2355_WIND.direction = bWSR_CurrentWindDirection ;
            FIFO_WS2355_AOP[FIFO_WS2355_AOP_PUSH].payload.data.WS2355_WIND.status = 1;
            FIFO_WS2355_AOP[FIFO_WS2355_AOP_PUSH].sent = false;
            FIFO_WS2355_AOP[FIFO_WS2355_AOP_PUSH].updated = millis();
            Serial.println("WS2355_WIND SENSOR");
            Serial.print("> Speed      ="); Serial.println(FIFO_WS2355_AOP[FIFO_WS2355_AOP_PUSH].payload.data.WS2355_WIND.speed, DEC);
            Serial.print("> Direction  ="); Serial.println(FIFO_WS2355_AOP[FIFO_WS2355_AOP_PUSH].payload.data.WS2355_WIND.direction, DEC);
            FIFO_WS2355_AOP_PUSH++;
            break;
          }
        default:
          {
            Serial.println("DISCARDED!");  // don't know this packet
            break;
          }
      }
    } else {
      Serial.print( "CRC ERROR" ); // bad checksum or packet header
    }
  ...
  if ( (uiICP_CapturedPeriod >= WSR_PERIOD_FILTER_MIN) && (uiICP_CapturedPeriod <= WSR_PERIOD_FILTER_MAX) )
  {
    ...
    if ( bICP_CapturedPeriodWasHigh )
    {
      ...
      if ( (uiICP_CapturedPeriod >= WSR_SHORT_PERIOD_MIN) && (uiICP_CapturedPeriod <= WSR_SHORT_PERIOD_MAX) )
      {
        ...
        //Serial.print(uiICP_CapturedPeriod);
        //Serial.print(" in ");
        //Serial.print(WSR_LONG_PERIOD_MIN);
        //Serial.print(" .. ");
        //Serial.println(WSR_LONG_PERIOD_MAX);
        //Serial.println("");
        WSR_RESET();
      }
    }
